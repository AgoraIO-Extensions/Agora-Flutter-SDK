...
//  skip: !(),
  ...
  testWidgets(
    'MediaEngine.destroyLoopbackAudioTrack',
    (WidgetTester tester) async {
      String engineAppId = const String.fromEnvironment('TEST_APP_ID',
          defaultValue: '<YOUR_APP_ID>');
      ...
      try {
        int trackId = 5;
        await mediaEngine.destroyLoopbackAudioTrack(
          trackId,
        );
      } catch (e) {
        if (e is! AgoraRtcException) {
          debugPrint(
              '[MediaEngine.destroyLoopbackAudioTrack] error: ${e.toString()}');
          rethrow;
        }
...
//  skip: !(),
  ...
  testWidgets(
    'MediaEngine.updateLoopbackAudioTrackConfig',
    (WidgetTester tester) async {
      String engineAppId = const String.fromEnvironment('TEST_APP_ID',
          defaultValue: '<YOUR_APP_ID>');
      ...
      try {
        int trackId = 5;
        LoopbackAudioTrackType configLoopbackType =
            LoopbackAudioTrackType.loopbackSystem;
        int configVolume = 5;
        String configDeviceName = "hello";
        String configAppName = "hello";
        int configProcessId = 5;
        LoopbackAudioTrackConfig config = LoopbackAudioTrackConfig(
          loopbackType: configLoopbackType,
          volume: configVolume,
          deviceName: configDeviceName,
          appName: configAppName,
          processId: configProcessId,
        );
        await mediaEngine.updateLoopbackAudioTrackConfig(
          trackId: trackId,
          config: config,
        );
      } catch (e) {
        if (e is! AgoraRtcException) {
          debugPrint(
              '[MediaEngine.updateLoopbackAudioTrackConfig] error: ${e.toString()}');
          rethrow;
        }
