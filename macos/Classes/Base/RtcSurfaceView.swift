...
class RtcSurfaceView: NSView {
    ...
    private func setupVideoCanvas(_ engine: AgoraRtcEngineKit) {
        ...
        surface = NSView(frame: CGRect(origin: CGPoint(x: 0, y: 0), size: bounds.size))
        addSubview(surface)
        canvas.view = surface
        if canvas.uid == 0 {
            engine.setupLocalVideo(canvas)
