...
class RtcEngineEventHandler {
  ...
  /// 
  ///
  /// Param [err] The error code returned by the SDK when the method call fails. For detailed error information and troubleshooting methods,
  /// see Error Code and Warning Code. If
  /// the SDK returns 0, then the method call is successful.
  ///
  ...
  ///
  /// Param [state] 
  /// The current connection state. For details, see ConnectionStateType.
  /// 
  /// Disconnected (1): The SDK is
  /// disconnected from Agora's edge server.
  /// Connecting (2): The SDK is
  /// connecting to Agora's edge server.
  /// Connected (3): The SDK is connected
  /// to Agora's edge server and has joined a channel.
  /// Reconnecting (4): The SDK keeps
  /// reconnecting to the Agora edge server.
  /// Failed 5: The SDK fails to connect
  /// to Agora's edge server or join the channel.
  /// 
  /// 
  ///
  /// Param [reason] 
  /// The reason for a connection state change. For details, see ConnectionChangedReason.
  /// 
  ///
  ...
  /// 
  ///
  /// Param [code] The event code of channel media relay. For
  /// details, see ChannelMediaRelayEvent. The event code of channel media relay:
  /// Disconnect (0): The user
  /// disconnects from the server due to a poor network connection.
  /// Connected(1): The user is connected
  /// to the server.
  /// JoinedSourceChannel (2): The user joins
  /// the source channel.
  /// JoinedDestinationChannel (3): The user
  /// joins the destination channel.
  /// SentToDestinationChannel(4): The SDK
  /// starts relaying the media stream to the destination channel.
  /// ReceivedVideoPacketFromSource (5): The
  /// server receives the video stream from the source channel.
  /// ReceivedAudioPacketFromSource (6): The
  /// server receives the audio stream from the source channel.
  /// UpdateDestinationChannel (7): The
  /// destination channel is updated.
  /// UpdateDestinationChannelRefused (8): The
  /// destination channel update fails due to internal reasons.
  /// UpdateDestinationChannelNotChange (9):
  /// The destination channel does not change, which means that the
  /// destination channel fails to be updated.
  /// UpdateDestinationChannelIsNil 10: The
  /// destination channel name is NULL.
  /// VideoProfileUpdate (11): The video profile
...
class RtcChannelEventHandler {
  ...
  /// The reason for a connection state change. For details, see ConnectionChangedReason.
  /// 
  /// Connecting (0): The SDK is
  /// connecting to Agora's edge server.
  /// JoinSuccess (1): The SDK has
  /// joined the channel successfully.
  /// Interrupted (2): The connection
  /// between the SDK and Agora's edge server is interrupted.
  /// BannedByServer (3): The
  /// connection between the SDK and Agora's edge server is banned by
  /// Agora's edge server.
  /// JoinFailed (4): The SDK fails to
  /// join the channel.
  /// LeaveChannel (5): The SDK has
  /// left the channel.
  /// InvalidAppId (6): The connection
  /// failed because the App ID is not valid. Please rejoin the channel
  /// with a valid App ID.
  /// InvalidChannelName (7): The
  /// connection failed because the channel name is not valid. Please
  /// rejoin the channel with a valid channel name.
  /// InvalidToken (8): The connection
  /// failed because the token is not valid. Typical reasons include:
  /// The App Certificate for the project is enabled in Agora Console,
  /// but you do not use a token when joining the channel. If you
  /// enable the App Certificate, you must use a token to join the
  /// channel.
  /// The uid specified when calling  to
  /// join the channel is inconsistent with the uid passed in when
  /// generating the token.
  /// 
  /// TokenExpired (9): The token has
  /// expired. You need to generate a new token from your server.
  /// RejectedByServer (10): The user
  /// is banned by the server.
  /// SettingProxyServer (11): The SDK
  /// tries to reconnect after setting a proxy server.
  /// RenewToken (12): The connection
  /// state changed because the token is renewed.
  /// ClientIpAddressChanged (13):
  /// The IP Address of the SDK client has changed, probably due to a
  /// change of the network type, IP address, or network port.
  /// KeepAliveTimeout (14): Timeout
  /// for the keep-alive of the connection between the SDK and the Agora
  /// edge server. The connection state changes to state.
  /// (15): The SDK has
  /// rejoined the channel successfully.
  /// (16): The connection between
  /// the SDK and the server is lost.
  /// (17): The connection
  /// state changes due to the echo test.
  /// 
  /// 
  ///
  /// Param [state] 
